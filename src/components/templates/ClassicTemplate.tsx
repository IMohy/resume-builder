...
END:VCARD`;
  ...
  const qrCodeTitle = personalInfo.qrLink ? personalInfo.qrTitle || t("qrCode.defaultTitle") : t("pdf.scanForContact");

  return (
    <div
      className={`bg-white text-black p-8 max-w-4xl mx-auto shadow-lg ${dir === "rtl" ? "text-right" : "text-left"}`}
    >
      {/* Header */}
      ...
      {personalInfo.summary && (
        <section className="mb-6">
          <h2 className="text-xl font-bold mb-2 border-b border-gray-200 pb-1">{t("form.summary")}</h2>
          <p className="text-gray-700 truncate text-wrap">{personalInfo.summary}</p>
        </section>
      )}

      {experience.length > 0 && (
        <section className="mb-6">
          <h2 className="text-xl font-bold mb-2 border-b border-gray-200 pb-1">{t("form.experience")}</h2>
      ...
      {education.length > 0 && (
        <section className="mb-6">
          <h2 className="text-xl font-bold mb-2 border-b border-gray-200 pb-1">{t("form.education")}</h2>
          <div className="space-y-4">
            {education.map((edu) => (
              <div key={edu.id}>
                <div className="flex justify-between">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {skills.length > 0 && (
          <section>
            <h2 className="text-xl font-bold mb-2 border-b border-gray-200 pb-1">{t("form.skills")}</h2>
        ...
        {languages.length > 0 && (
          <section>
            <h2 className="text-xl font-bold mb-2 border-b border-gray-200 pb-1">{t("form.languages")}</h2>
            <ul className="list-disc list-inside space-y-1">
              {languages.map((language) => (
                <li key={language.id}>
                  {language.name}
                  <span className="text-gray-600 ml-2">({t(`proficiencyLevels.${language.proficiency}`)})</span>
                </li>
              ))}
            </ul>
          </section>
      ...
      <footer className="mt-8 pt-4 border-t border-gray-300 flex justify-between items-center">
        <div className={dir === "rtl" ? "mr-auto" : "ml-auto"}>
          <QRCode value={qrCodeValue} size={80} title={qrCodeTitle} />
        </div>
      </footer>
    </div>
  );
